...
# 学生2申请加入团队
echo ">>> 学生2申请加入团队"
APPLICATION_DATA="{\"teamId\":$TEAM_ID,\"studentId\":$STUDENT2_ID,\"message\":\"我想加入团队\"}"
APP_RESULT=$(curl -s -X POST -H "Content-Type: application/json" -H "token: $STUDENT2_TOKEN" \
  -d "$APPLICATION_DATA" \
  "$BASE_URL/teamApplication/add")
...
if echo "$APP_RESULT" | grep -q '"code":"200"'; then
    ...
    # 获取申请ID
    sleep 2
    APPS=$(curl -s -H "token: $ADMIN_TOKEN" "$BASE_URL/teamApplication/selectAll?teamId=$TEAM_ID")
    ...
    if [ ! -z "$APP_ID" ]; then
        echo ""
        echo ">>> 队长审核加入申请"
        APPROVE=$(curl -s -X PUT -H "Content-Type: application/json" -H "token: $STUDENT1_TOKEN" \
          -d "{\"id\":$APP_ID,\"status\":\"已通过\"}" \
          "$BASE_URL/teamApplication/update")
...
echo "==================== 第五部分：团队信息修改 ===================="
# 更新团队信息
UPDATE_DATA="{\"id\":$TEAM_ID,\"teamName\":\"更新后的团队名称\",\"status\":\"已满员\"}"
...
echo "==================== 第七部分：团队搜索和筛选 ===================="
# 按团队名称搜索
test_api "按名称搜索团队" "/team/selectAll?teamName=测试" "GET" "" "$ADMIN_TOKEN"
test_api "搜索不存在的团队" "/team/selectAll?teamName=不存在的团队" "GET" "" "$ADMIN_TOKEN"
...
test_api "分页查询团队申请" "/teamApplication/selectPage?pageNum=1&pageSize=10" "GET" "" "$ADMIN_TOKEN"
test_api "查询指定团队的申请" "/teamApplication/selectAll?teamId=$TEAM_ID" "GET" "" "$ADMIN_TOKEN"
...
echo ">>> 创建多个团队用于分页测试"
for i in {1..3}; do
    RAND_USER="page_test_$i$(date +%s)"
    curl -s -X POST -H "Content-Type: application/json" \
      -d "{\"username\":\"$RAND_USER\",\"password\":\"123456\",\"role\":\"USER\"}" \
      "$BASE_URL/register" > /dev/null
    ...
    TOPIC_TITLE="分页测试选题$i"
    curl -s -X POST -H "Content-Type: application/json" -H "token: $USER_TOKEN" \
      -d "{\"title\":\"$TOPIC_TITLE\",\"description\":\"测试\",\"category\":\"技术创新\"}" \
      "$BASE_URL/topic/add" > /dev/null
    sleep 1
    
    USER_TOPICS=$(curl -s -H "token: $USER_TOKEN" "$BASE_URL/topic/my?pageNum=1&pageSize=1")
    USER_TOPIC_ID=$(echo $USER_TOPICS | grep -o '"id":[0-9]*' | head -1 | grep -o '[0-9]*')
    
    curl -s -X POST -H "Content-Type: application/json" -H "token: $USER_TOKEN" \
      -d "{\"topicId\":$USER_TOPIC_ID,\"teamName\":\"分页测试团队$i\"}" \
      "$BASE_URL/team/add" > /dev/null
